...
export default async function twoFARoutes(fastify: FastifyInstance) {
    ...
    fastify.post('/setup', { preValidation: [fastify.authenticate] }, async (request: any, reply) => {
        ...
        const updated2FASecret: any = await fastify.dbClient.post('/user/update2FASecret', { userid, twoFASecret: secret.base32 });
        if (updated2FASecret.error) return reply.code(500).send({error: updated2FASecret.error});
    ...
    fastify.post('/verify-setup', { preValidation: [fastify.authenticate] }, async (request: any, reply) => {
        ...
        const updated2FAEnabled: any = await fastify.dbClient.post('/user/update2FAEnabled', { userid });
        if (updated2FAEnabled.error) return reply.code(500).send({error: updated2FAEnabled.error});
